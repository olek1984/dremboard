...
class UserList {
	...
	switch ( $link_type ) {
		...
		* @see http://wordpress.org/support/topic/309878 */
		...
		//!!!
		$currentloggedinuserid = bp_loggedin_user_id();
		if ( $user->user_id == $currentloggedinuserid )
		{
		...
		else 
		{
			...
			'follower_id' => $currentloggedinuserid
				);
			
				$html .= '<span class="name">'. $user->display_name . " ".  bp_follow_get_only_follow_button( $args1 ). '</span>'; 
...
//!!!			$html .= '<span class="name">'. $user->display_name . '</span>'; 
	...
	function get_blog_users( $roles ) {
		...
		//die;
		global $wpdb;
        $bmp_table = $wpdb->base_prefix . "rt_rtm_media";
        $resultusers = array();
		foreach ($users as $usernow)
		{
			$trmsql = "select media_author from $bmp_table where media_author = ".$usernow->user_id." limit 1";
			$trmresult = $wpdb->get_var($trmsql);
			if (empty($trmresult))
			{
				continue;
