 ...
 * @version 1.0
 */
function bpb_check_message_receipients( $recipients ) {
	$cui = bp_loggedin_user_id();
	$_list = bpb_get_blocked_users_list_by_type( $cui, BPB_MESSAGE );
	...
	foreach ( (array) $recipient_ids as $user_id ) {
		$list = bpb_get_blocked_users_list_by_type( $user_id, BPB_MESSAGE );
 ...
 * @version 1.0
 */
function bpb_before_message_send( $message ) {
