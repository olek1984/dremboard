...
function bpb_adjust_user_query( &$data ) {
	...
	$_list = bpb_get_blocked_users_list_by_type( get_current_user_id() );
...
function bpb_adjust_total_count( $count ) {
	...
	$list = count( bpb_get_blocked_users_list_by_type( get_current_user_id() ) );
...
function bpb_adjust_latest_update( $update_content ) {
	if ( is_user_logged_in() ) {
		$list = bpb_get_blocked_users_list_by_type( bp_get_member_user_id() );
...
function bpb_adjust_mentions( $content, $activity ) {
	...
	// My list
	$my_list = bpb_get_blocked_users_list_by_type( get_current_user_id() );
	...
	if ( ! empty( $usernames ) ) {
		...
		foreach( (array) $usernames as $user_id => $username ) {
			...
			$list = bpb_get_blocked_users_list_by_type( $user_id );
