...
/*	
	if ( !$bp->loggedin_user->is_super_admin && bp_get_activity_user_id() == $bp->loggedin_user->id && 'activity_update' != bp_get_activity_type() )
		return;
*/
	if ( !$bp->loggedin_user->is_super_admin && bp_get_activity_user_id() != $bp->loggedin_user->id)
		return;
...
function etivite_bp_edit_activity_action_edit_router() {
	...
	if ( isset( $_POST['save_changes'] ) ) {
        ...
        if ( isset( $_POST['activity_text'] ) && !empty( $_POST['activity_text'] ) ){
            $content_tail = strstr($new_content, '<ul');
            if($content_tail == ""){
                $new_content = $_POST['activity_text'];
            }else{
                $content_head = '<div class="rtmedia-activity-container"><div class="rtmedia-activity-text">'.$_POST['activity_text'].'</div>';
                $new_content = $content_head.$content_tail;
                $new_content = preg_replace("/\'/", '"', $new_content);
            }
        }
        if ( isset($_POST['media_category']) && is_array($_POST['media_category']) 
                && isset($_POST['media_id']) && is_array($_POST['media_id'])){
            $rtmedia_model = new RTMediaModel();
            foreach ($_POST['media_id'] as $key => $media_id){
                $category = $_POST['media_category'][$key];
                $rtmedia_model->set_media_category ($media_id, $category);
            }
        }
...
function etivite_bp_edit_the_activity_text() {
	echo etivite_bp_edit_get_the_activity_text();
}
	function etivite_bp_edit_get_the_activity_text() {
		global $activity_edit_template;
        $content = $activity_edit_template['activities'][0]->content;
        $activity_text = strip_tags(strstr($content, '<ul', true));
        if(strstr($content, '<ul', true) == false){
            $activity_text = $content;
        }
		return apply_filters( 'etivite_bp_edit_get_the_activity_text', stripslashes( esc_attr( $activity_text ) ) );
	}
