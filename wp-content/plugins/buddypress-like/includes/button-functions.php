...
function bp_flag_button() {
    $id = bp_get_activity_id();
    $btn = '<span class="bp-flag-button active">
        <a class="button item-button" title = "Flag this active" onClick="flag_activity('.$id.', '.$id.');">Flag</a>
    </span >';
    echo $btn;
}
...
add_filter( 'bp_activity_entry_meta' , 'bp_flag_button', 1000 );

add_action( 'bp_before_blog_single_post' , 'bp_like_button' , 1000 );

add_filter( 'bp_activity_comment_options' , 'bp_like_button', 1000 );

add_filter( 'bp_activity_entry_share' , 'bp_like_button', 1000 );

add_filter( 'bp_single_media_like' , 'bp_like_button', 1000 );
...
function bplike_activity_button() {
    ...
    $bp_like_rid = rtmedia_activity_id(rtmedia_id());

	if (isset($bp_like_rid)) {
    	$bp_like_id = rtmedia_activity_id(rtmedia_id());
    ...
    } else if ( empty( $bp_like_comment_id ) ) {
    ...
    $bp_like_css .= " bp-secondary-action";
