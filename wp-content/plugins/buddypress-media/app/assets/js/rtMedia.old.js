            ...
            rtMagnificPopup = jQuery(selector).magnificPopup({
                ...
                callbacks: {
                    ajaxContentAdded: function() {
                        ...
                        //alert(current_media.html());
                        //alert('0000000001');
                        //alert(jQuery(li).html());
                        //newid = jQuery(li).find('p>a').attr('id');
                        //alert(newid);
                        if (newcounter1 == 0)
                        {
                        	//alert('0000000000');
                        	newid = jQuery(li).find('p>a').attr('id');
                        	newclass = 'need-popup-'+newid;
                        	//alert(jQuery(li).find('p>a').attr('class'));
                        	//alert(newclass);
                        	var usernameNum3 = jQuery(li).find('p>a').attr('class').match(newclass);
                        	if (usernameNum3 == newclass)
                        	{
                        		getnewid = newid;
                        		alert(mfp.items);
                        		var result_item = [];
                        		for(var key in mfp.items)
                        		{
                        			newclass1 = 'http://127.0.0.1/members/'+getnewid;
                        			if(Cts.indexOf("Text") >= 0 )
									{  
    
									}  
                        			var usernameNum3 = mfp.items[key];
                        			alert(usernameNum3);
                        			//result_item.push(mfp.items[key]);
                        		}
                        		alert(result_item);
                        		//mfp.open(options);
                                    //mfp.items.push({src: jQuery("."+newclass);});
									//apply_rtMagnificPopup(selector);
                                	//mfp.updateItemHTML();                                    
                        	}
                        	//alert(getnewid);
                        	newcounter1 = newcounter1 +1;
                        }
...
jQuery('document').ready(function($) {
    ...
    $("li.rtmedia-list-item p a").each(function(e) {
        ...
        //!!!!!!!
        var usernameNum = $(this).attr('href').match(/members\/([0-9a-zA-Z]+)\/media/)[1];
    ...
    if (typeof(rtmedia_lightbox_enabled) != 'undefined' && rtmedia_lightbox_enabled == "1") {
        ...
        //apply_rtMagnificPopup('.rtmedia-list-media,.rtmedia-activity-container ul.rtmedia-list,#bp-media-list, .bp-media-sc-list, li.media.album_updated ul,ul.bp-media-list-media, div.bp_media_content');
    }
