...
class RTMediaActivity {
    ...
    function create_activity_html ($desc=false, $img_full=true) {
		if ($desc){
			$this->activity_text = $desc;
		}
        ...
        if ( ! empty ( $this->activity_text ) ) {
            ...
            $tmp = $this->activity_text;
            if(substr($tmp, -4) == ".jpg" || substr($tmp, -4) == ".png" || substr($tmp, -4) == ".gif")
            	$tmp = substr($this->activity_text, 0, -4);
            $html .= $tmp;
            $html .= '</div>';
        }
       ...
       $count = 0;
        foreach ( $media_details as $media ) {
            ...
            if ($img_full==false){
				...
				$li_content .= '<div class="rtmedia-item">';
			}
			
            $li_content .= $this->media ( $media, $img_full );

            $li_content .= '</div>';
//!!!!!!!!
			//var_dump($media->media_title);
			$media->media_title = str_ireplace('.jpg','',$media->media_title);
			$media->media_title = str_ireplace('.jpeg','',$media->media_title);
			$media->media_title = str_ireplace('.png','',$media->media_title);
			$media->media_title = str_ireplace('.gif','',$media->media_title);
            //var_dump("1212121");
            //var_dump($media->media_title);
    ...
    function media ( $media, $img_full=true) {
...
//			}
	     global $rtmedia;
            if ( $media->media_type == 'photo' ) {
                ...
                $html = '<video '.$poster.' src="' . wp_get_attachment_url ( $media->media_id ) . '" width="' . $rtmedia->options[ "defaultSizes_video_activityPlayer_width" ] . '" height="' . $rtmedia->options[ "defaultSizes_video_activityPlayer_height" ] . '" type="video/mp4" class="wp-video-shortcode" id="rt_media_video_' . $media->id . '" controls="controls" preload="true"></video>';
