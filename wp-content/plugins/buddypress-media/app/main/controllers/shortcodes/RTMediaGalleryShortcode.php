...
class RTMediaGalleryShortcode {
    ...
    static function render ( $attr ) {
        if ($attr['media_type'] == 'photo'){
            $template_url = add_query_arg(array("action" => 'rtmedia_get_template', "origin_page" => "drems", "template" => apply_filters('rtmedia_backbone_template_filter',"media-gallery-item")),admin_url("admin-ajax.php"));
        }else if ($attr['media_type'] == 'dremboard'){
            $template_url = add_query_arg(array("action" => 'rtmedia_get_template', "origin_page" => "dremboard", "template" => "album-gallery-item"),admin_url("admin-ajax.php"));
        }
        wp_localize_script ( 'rtmedia-backbone', 'template_url', $template_url );
