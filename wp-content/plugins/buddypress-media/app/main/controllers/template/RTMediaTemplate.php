...
//var_dump("4037");
		...
		echo "<div class='rtmedia_gallery_wrapper'>";
                ...
                /*
                //!!!!! 
                if ($shortcode_attr['attr']['media_type'] == 'dremboard')
                {
                	$gallery_template = 'dremboard-gallery';
                }
                */
		...
		echo "<div class='rtmedia_gallery_wrapper'>";
                ...
                //!!!!!
                global $for_shortcode_dremboard;
                $for_shortcode_dremboard = $shortcode_attr[ 'attr' ];
                //!!!!! include $this->locate_template ( $gallery_template );
                include $this->locate_template ( $gallery_template,false,false, $shortcode_attr[ 'attr' ] );
                //var_dump("4046");
		echo "</div>";
    ...
    function add_hidden_fields_in_gallery () {
        ...
        if (isset($_REQUEST['pcat'])){
            $return_str.= '<input name="pcat" value="' . $_REQUEST['pcat'] . '" type="hidden" />';
        }
        if (isset($_REQUEST['ps'])){
            $return_str.= '<input name="ps" value="' . $_REQUEST['ps'] . '" type="hidden" />';
        }
        $return_str.= '<input name="origin_page" value="' . $pagename . '" type="hidden" />';
        echo $return_str;
    }
    ...
    function json_output () {
        ...
        if ( $rtmedia_query->media ) {
            foreach ( $rtmedia_query->media as $key => $media ) {
                ...
                if ($rtmedia_query->media_query['media_type'] == 'photo'){
                    $media_array[ $key ]->media_category = get_rtmedia_category ( $media->media_id );
                }
                $media_array[ $key ]->activity_like = view_like_status( $media->activity_id , 'activity', false );
                $test = '';
                if ($rtmedia_query->media_query['media_type'] == 'album'){
                    $media_array[ $key ]->rt_author_link = get_rtmedia_author_link ( $media->media_author );
                    $media_array[ $key ]->rt_author_avatar = get_rtmedia_author_avatar ( $media->media_author );;
                    $media_array[ $key ]->rt_author_name_link = rtmedia_author_name ( $show_link = true, $echo = false, $author_id = $media->media_author );
                    $media_array[ $key ]->rt_album_count = rtmedia_album_count( $echo = false, $mediaid = $media->media_id );
                    $media_array[ $key ]->rt_album_thumb = rtmedia_image ( $size = 'rt_media_thumbnail', $id = $media->id ,$recho = false );
                }
            }
        }
    ...
    function save_album_edit () {
		...
		do_action ( 'rtmedia_after_update_album', $album[ 0 ]->id, $state );
            } elseif ( isset ( $_POST[ 'media_action' ] ) && isset ( $_POST[ 'media_action' ] ) == "move" ) {
//                            print_r($_POST);die;
...
//                        do_action('rtmedia_before_delete_media',$rtmedia_query->media[ 0 ]->id);
	...
	$nonce = $_REQUEST[ 'rtmedia_delete_album_nonce' ];
        ...
        if (preg_match('/.*\/show\//', $request_url)){
            wp_safe_redirect ( '/dremboard/' );
            exit;
        }
        if(isset($rtmedia_query->media_query['context']) && $rtmedia_query->media_query['context'] == "group") {
            global $bp;
            $group_link = bp_get_group_permalink($bp->groups->current_group);
            wp_safe_redirect ( trailingslashit( $group_link ) . RTMEDIA_MEDIA_SLUG . '/dremboard/' );
        } else {
            wp_safe_redirect ( trailingslashit( get_rtmedia_user_link ( get_current_user_id () ) ) . RTMEDIA_MEDIA_SLUG . '/dremboard/' );
        }
        exit;
    }
...
//                    print_r($album_contents); die;
    ...
    function check_return_comments () {
        ...
        if ( isset ( $rtmedia_query->action_query->id ) && count ( $_POST ) ) {
            ...
            if ( wp_verify_nonce ( $nonce, 'rtmedia_comment_nonce' ) ) {
            	...
            	if(isset($_POST['confirm_flag']) && $_POST['confirm_flag'] == 'true'){
            		
            		$mediaModel = new RTMediaModel();
					$result = $mediaModel->get ( array( 'id' => $rtmedia_query->action_query->id ) );
            		rtmedia_comments(true, $result[ 0 ]->media_id);
                ...
                if (($picture_flag == false) && empty ( $_POST[ 'comment_content' ] ) ) {
                    return false;
                }
                $comment = new RTMediaComment();
                $attr = $_POST;
                
                $content_data = $_POST['comment_content'];
				...
				if ( $picture_flag ){
					...
					$uploaded['description'] = $_POST['comment_content'];
					...
					//$activity_id = $mediaObj->insert_activity ( $media[ 0 ]->media_id, $media[ 0 ],  $_POST['comment_content']);
					$activity = new RTMediaActivity ( $media[ 0 ]->id, 0 );
			        $activity_content = $activity->create_activity_html ($_POST['comment_content'], true);
			        $content_data = $activity_content;
				}
                $attr['comment_content'] = $content_data;
                $mediaModel = new RTMediaModel();
                $result = $mediaModel->get ( array( 'id' => $rtmedia_query->action_query->id ) );
                ...
                if ( $result[ 0 ]->activity_id != NULL ) {
                    ...
                    if ( function_exists ( 'bp_activity_new_comment' ) ) {
                       $comment_activity_id = bp_activity_new_comment ( array( 'content' => $content_data, 'activity_id' => $result[ 0 ]->activity_id ) );
                    }
                }
                if(!empty($comment_activity_id)){
                    ...
                    if ($picture_flag){
						$mediaObj->model->update (
				        	array( 'activity_id' => $comment_activity_id ), array( 'id' => $media[ 0 ]->id )
				    	);
					}
                }
    ...
    function sanitize_gallery_attributes ( &$attr ) {
        ...
        if ( isset ( $attr[ 'media_type' ] ) ) {
            ...
            //!!!!!
			else if(strtolower ( $attr[ 'media_type' ] ) == 'dremboard' ){
                $flag = $flag && true;
            }            
            else
    ...
    static function locate_template ( $template = false, $context = false, $url = false, $attr = false ) {
        $located = '';
        //var_dump("4111");
        //var_dump($template);
        if ( ! $template ) {
			...
			if (( is_rtmedia_album_gallery() ) || ( is_rtmedia_dremboard_gallery() ) )
			{
				if ( is_rtmedia_album_gallery() ) {
					//var_dump("1004");
					$template = 'album-gallery';
