...
function rtmedia () {
    global $rtmedia_query;
//!!!!! --- \www\wp-content\plugins\buddypress-media\app\main\routers\query\RTMediaQuery.php (841)     function rtmedia () {
...
function rtmedia_title () {
    ...
    if ( $rtmedia_backbone[ 'backbone' ] ) {
        ...
        $new_media_title_all = $rtmedia_media->media_title;
        
		$new_media_title_all = str_ireplace('.jpg','',$new_media_title_all);
		$new_media_title_all = str_ireplace('.jpeg','',$new_media_title_all);
		$new_media_title_all = str_ireplace('.png','',$new_media_title_all);
		$new_media_title_all = str_ireplace('.gif','',$new_media_title_all);
				
        return $new_media_title_all;
    }
}

function rtmedia_category () {
    ...
    if ( $rtmedia_backbone[ 'backbone' ] ) {
    	...
    	$category = $rtmedia_model->get_media_category( $media_id );
		$category_str = get_term( $category, 'category' );
...
function get_rtmedia_gallery_title () {
	...
	$photos_title =  __( 'All Drēms','rtmedia' );
...
function get_rtmedia_dremboard_desc () {
    global $rtmedia_query;
    $desc = false;
    ...
    if( isset( $rtmedia_query->query['media_type'] ) &&  $rtmedia_query->query['media_type'] == "album"
        && isset( $rtmedia_query->media_query['album_id'] )  &&  $rtmedia_query->media_query['album_id'] != ""  ) {
        $id = $rtmedia_query->media_query['album_id'];
        $desc = rtmedia_description(false, $id);
    }
    return $desc;
}

function get_rtmedia_dremboard_author () {
    global $rtmedia_query;
    $author = false;
    ...
    if( isset( $rtmedia_query->query['media_type'] ) &&  $rtmedia_query->query['media_type'] == "album"
        && isset( $rtmedia_query->media_query['album_id'] )  &&  $rtmedia_query->media_query['album_id'] != ""  ) {
        $id = $rtmedia_query->media_query['album_id'];
        $author = rtmedia_author_profile_pic(true, false);
    }
    return $author;
}
...
function get_rtmedia_title($id) {
    $rtmedia_model = new RTMediaModel();
    $title = $rtmedia_model->get( array('id' => $id, 'ps_flag' => false ) );
    //!!!!!!!! return $title[0]->media_title;
    
    $my_title_now = $title[0]->media_title;
...
function rtmedia_author_name ( $show_link = true, $echo = true, $author_id = false ) {
    ...
    if ($author_id){
        ...
        $media_author = $rtmedia_media->media_author;
    }
    ...
    $author_name = '';
        ...
        if ( $show_link ) {
            $author_name .= "<a href='" . get_rtmedia_user_link ( $media_author ) . "' title='" . rtmedia_get_author_name ( $rtmedia_media->media_author ) . "'>";
        }
        $author_name .= rtmedia_get_author_name ( $media_author );
        if ( $show_link ) {
            $author_name .= "</a>";
        }
...
/**
 * echo avatar link of the media author
 * @global type $rtmedia_media
 */
function rtmedia_author_avatar ( $author_id = false ) {

    global $rtmedia_backbone;

    if ( $rtmedia_backbone[ 'backbone' ] ) {
        echo '<%= rt_author_avatar %>';
    } else {
        echo get_rtmedia_author_avatar ( $author_id );
    }
}

function get_rtmedia_author_avatar ( $author_id = false ){
    if (!$author_id){
        global $rtmedia_media;
        $author_id = $rtmedia_media->media_author;
    }
    return bp_core_fetch_avatar( array( 'item_id' => $author_id, 'no_grav' => true, 'html' => false ));
}

/**
 * echo link of the media author with display name
 * @global type $rtmedia_media
 */
function rtmedia_author_link ( $author_id = false ) {
    ...
    if ( $rtmedia_backbone[ 'backbone' ] ) {
        ...
        echo get_rtmedia_author_link ( $author_id );
    }
}

function get_rtmedia_author_link ( $author_id = false ){
    if (!$author_id){
        global $rtmedia_media;
        $author_id = $rtmedia_media->media_author;
    }
...
function rtmedia_author_display_name ( $author_id = false ) {

    global $rtmedia_backbone;

    if ( $rtmedia_backbone[ 'backbone' ] ) {
        echo '<%= rt_author_display_name %>';
    } else {
        echo get_rtmedia_author_display_name ( $author_id );
    }
}

function get_rtmedia_author_name ( $author_id = false ){
    if (!$author_id){
        global $rtmedia_media;
        $author_id = $rtmedia_media->media_author;
    }
    return bp_core_get_user_displayname( $author_id );
}
...
function rtmedia_media ( $size_flag = true, $echo = true, $media_size = "rt_media_single_image" ) {
    ...
    if ( $rtmedia_backbone[ 'backbone' ] ) {
    	$html = "<img src='<%= guid %>' alt='<%= post_name %>' style='max-width: 96%;' class='template photo'/>";
    	...
    	$html .= "<div class='rtm-mejs-video-container template video' style=''>";
	    $html .= '<video src="<%= guid %>" ' . $size . ' type="video/mp4" class="wp-video-shortcode" id="bp_media_video_<%= media_id %>" controls="controls" preload="true"></video>';
	    $html .= '</div>';
	    $size = ' width="230px" height="30" ';
        if ( ! $size_flag )
            $size = '';
        $html .= '<audio src="<%= guid %>" ' . $size . ' type="audio/mp3" class="wp-audio-shortcode template music" id="bp_media_audio_<%= media_id %>" controls="controls" preload="none"></audio>';
    } else {
	    if ( isset ( $rtmedia_media->media_type ) ) {
	        if ( $rtmedia_media->media_type == 'photo' ) {
	            ...
	            $html = "<img src='" . $src[ 0 ] . "' alt='".$rtmedia_media->post_name."' style='max-width: 96%;'/>";
...
//var_dump("8060");
    ...
    if ( $id ) {
    	//var_dump("8062");
        $model = new RTMediaModel();
        $media = $model->get_media ( array( 'id' => $id, 'ps_flag' => false ), false, false );
        if ( isset ( $media[ 0 ] ) )
...
function rtmedia_album_image ( $size = 'thumbnail', $id = false) {
    ...
    if(isset($rtmedia_query->query['context_id']) && isset( $rtmedia_query->query['context'] ) && $rtmedia_query->query['context'] != "group" ){
		$URL = $_SERVER['REQUEST_URI'];
		if(strpos($URL, 'album') !== FALSE){
	        $media = $model->get_media ( array( 'album_id' => $id, 'media_type' => 'video', 'media_author' => $rtmedia_query->query['context_id'] , 'ps_flag' => false ), 0, 1 );
	        return $media[ 0 ]->media_id;
	    } else {
	        $media = $model->get_media ( array( 'album_id' => $id, 'media_type' => 'photo', 'media_author' => $rtmedia_query->query['context_id'] , 'ps_flag' => false ), 0, 1 );
	    }
    } else {
	if( isset( $rtmedia_query->query['context_id'] ) && isset( $rtmedia_query->query['context'] ) && $rtmedia_query->query['context'] == "group" ) {
	    $media = $model->get_media ( array( 'album_id' => $id, 'media_type' => 'photo', 'context_id' => $rtmedia_query->query['context_id'] , 'ps_flag' => false ), 0, 1 );
	} else {
	    $media = $model->get_media ( array( 'album_id' => $id, 'media_type' => 'photo', 'ps_flag' => false ), 0, 1 );
	}
    }
 ...
 * @global type $rtmedia_media
 */
function rtmedia_description ( $echo = true, $id = false) {
    global $rtmedia_media;
    $media_id = false;
    
    if($id){
        $rtmedia_model = new RTMediaModel();
        $media = $rtmedia_model->get( array('id' => $id, 'ps_flag' => false ) );
        $media_id = $media[0]->media_id;
    }
    
    $desc = get_post_field("post_content", ($media_id)?$media_id:$rtmedia_media->media_id);
...
function rtmedia_view_count(){
    global $rtmedia_media, $rtmedia_backbone;
    if ( $rtmedia_backbone[ 'backbone' ] ) {
        echo '<%= views %>';
    }else{
        echo $rtmedia_media->views;
    }
}

function update_rtmedia_view_count(){
    global $rtmedia_media;
    $rtmedia_model = new RTMediaModel();

    $media_table_id = $rtmedia_media->id;
    $views = $rtmedia_media->views;
    if ($views < 0) {
        $views = 1;
    }else{
        $views += 1;
    ...
    $rtmedia_model->set_media_views( $media_table_id, $views );
}

function update_rtmedia_view_count_by_activity_id($activity_id) {
    $rtmedia_model = new RTMediaModel();
    $rtmedias = $rtmedia_model->get_media(array('activity_id' => $activity_id, 'ps_flag' => false), false, false);
    foreach ($rtmedias as $rtmedia) {
        ...
        $rtmedia_model->set_media_views( $id, $views );
    }
}

function rtmedia_album_count( $echo = true, $mediaid=false ){
    global $wpdb, $rtmedia_media;
    
    if($mediaid){
    	$media_id = $mediaid;
    	$rtmedia_id = rtmedia_id ($media_id);
    }else{
    	$media_id = $rtmedia_media->media_id;
    ...
    $model = new RTMediaModel();
    $count = $model->get_media ( array( 'media_type' => 'photo', 'album_id' => $rtmedia_id, 'ps_flag'=> false ), false, false, 'media_id desc', true ); 
    if ($echo)
        echo $count;
    else
        return $count;
 ...
 */
function rtmedia_comments ( $echo = true, $mediaid=false ) {
    global $wpdb, $rtmedia_media;
    if($mediaid){
    	$media_id = $mediaid;
    	$rtmedia_id = rtmedia_id ($media_id);
    ...
    $comments = $wpdb->get_results ( "SELECT * FROM $wpdb->comments WHERE comment_post_ID = '" . $media_id . "'", ARRAY_A );
...
function rmedia_single_comment ( $comment ) {
    ...
    $html .= '<div class="rtmedia-comment-content">' . make_clickable( $comment[ 'comment_content' ] ) . '</div>';
...
function is_rtmedia_dremboard_gallery () {
    global $rtmedia_query;
    if ( $rtmedia_query )
    return $rtmedia_query->is_dremboard_gallery ();
    else
    return false;
}
...
function rtmedia_vedio_editor_content() {
    ...
    if ( isset($rtmedia_query->media) && !is_array($rtmedia_query->media))
        return;
    if ( isset($rtmedia_query->media[ 0 ]->media_type) && $rtmedia_query->media[ 0 ]->media_type == 'video' ) {
        $media_id = $rtmedia_query->media[ 0 ]->media_id;
        $thumbnail_array = get_post_meta($media_id, "rtmedia_media_thumbnails", true);
...
function rtmedia_add_album_selection_field( $media_type ){
    ...
    <div class="rtmedia-edit-change-album">
        ...
        <?php if( isset( $rtmedia_query->query['context']) && $rtmedia_query->query['context'] == 'group' ){
           ...
           $album_list = rtmedia_user_album_list ( $get_all = false, $selected_album_id = $curr_album_id, $blank_flag = true );
        }
...
function rtmedia_comment_form () {
    if(is_user_logged_in()) {
    	...
    	if ((bp_follow_is_following($display_user_id,$logged_in_user_id)) || ($display_user_id == $logged_in_user_id))
    	{
    ?>
    <form method="post" id="rt_media_comment_form" class="rt_media_comment_form" action="<?php echo get_rtmedia_permalink ( rtmedia_id () ); ?>comment/">
            <div style="position:relative;">
                <textarea style="width:100%" placeholder="<?php _e( 'Type Comment...', 'rtmedia' ); ?>" name="comment_content" id="comment_content" class="helper-input helper-input-box has_mark"></textarea>
        		<div class="ac-file">
					...
					<input type='hidden' name='image_data' id='image_data' val=''/>
				</div>
            </div>
    	...
    	<script>
    		init_comment_file_in_container(jQuery("#rt_media_comment_form"));
    		add_input_for_bp_user(jQuery("#rt_media_comment_form #comment_content"));
    	</script>
    <?php
    	}
    }
...
function rtmedia_user_album_list ( $get_all = false, $selected_album_id = false, $show_blank = false ) {
    global $rtmedia_query;
    $model = new RTMediaModel();
    ...
    $album_objects = $model->get_media ( array( 'media_author' => get_current_user_id (), 'media_type' => 'album', 'source' => 'dremboard' ), false, 'context' );
    $option_group = "";
    $profile_option = "";
    if ( $album_objects ) {
        if ($show_blank)
            $profile_option .= '<option value="" >-----  select  -----</option>';
        foreach ( $album_objects as $album ) {
        	/*
        	if (is_rtmedia_dremboard_gallery())
        	{
        		if ($album->source != 'dremboard')
        		{
        			continue;
        		}
        	}
...
//                    $option_group .= '<option value="' . $album->id . '">' . $album->media_title . '</option>';
    ...
    //!!!!! $option.= "<optgroup label='".__("Profile Memories","rtmedia")." ' value = 'profile'>$profile_option</optgroup>";
    if (is_rtmedia_dremboard_gallery())
    {*/
    	$option.= "<optgroup label='".__("Profile Drēmboards","rtmedia")." ' value = 'profile'>$profile_option</optgroup>";
    /*}
    else 
    {
    	$option.= "<optgroup label='".__("Profile Memories","rtmedia")." ' value = 'profile'>$profile_option</optgroup>";
    ...
    $option.="<optgroup label='".__("Group Memories","rtmedia")."' value = 'group'>$option_group</optgroup>";
    if ( $option )
        return $option;
    else
        return false;
...
function rtmedia_create_album ( $options) {
    ...
    if ( $display === true ) {
        ...
        if ( is_rtmedia_album_gallery() )
        {
        	$options[] = "<a href='#rtmedia-create-album-modal' class='rtmedia-reveal-modal rtmedia-modal-link'  title='".  __( 'Create New Memory', 'rtmedia' ) ."'><i class='rtmicon-plus-circle'></i>" . __('Add Memory') . "</a>";
        }
        //!!!!! $options[] = "<a href='#rtmedia-create-dremboard-modal' class='rtmedia-reveal-modal rtmedia-modal-link'  title='".  __( 'Create New Drem', 'rtmedia' ) ."'><i class='rtmicon-plus-circle'></i>" . __('Add Dremboard') . "</a>";
        if (is_rtmedia_dremboard_gallery()) 
        {
    ...
    ?>
        <div class="mfp-hide rtmedia-popup rtmedia-create-dremboard-modal" id="rtmedia-create-album-modal">
            <div id="rtm-modal-container">
                <h2 class="rtm-modal-title"><?php //!!! _e('Create New Album', 'rtmedia'); 
    			if ( is_rtmedia_album_gallery() )
    			{
                ...
                <?php //!!!!!  ?>
                    ...
                    <label for="rtmedia_album_name"><?php //!!!��_e('Album Title : ', 'rtmedia');
    				_e('Title: ', 'rtmedia');
    			?></label>
    			</p>
                    ...
                    <label for="rtmedia_album_name">
                <?php //!!!��_e('Album Title : ', 'rtmedia');
    				if ( is_rtmedia_album_gallery() ) 
    				{
	    				echo '<p>Personal Memory:</p>';
	    				echo "<input type='checkbox' id='rtmedia_select_album_privacy' name='rtmedia_select_album_privacy' value='YES'>Make it your personal memory so only you can see";
					} else {
    ...
    global $rtmedia_query;
     ...
     ?>
        <div class="rtmedia-merge-container rtmedia-popup mfp-hide" id="rtmedia-merge">
           <div id="rtm-modal-container">
               <h2 class="rtm-modal-title"><?php //!!! _e( 'Merge Album', 'rtmedia' );
				//!!!!! _e( 'Merge Memory', 'rtmedia' );
				_e( 'Merge Item', 'rtmedia' );
     			?></h2>
               <form method="post" class="album-merge-form" action="merge/">
                   <?php //!!! _e('Select Album to merge with : ','rtmedia');
     				//!!!!! _e('Select Memory to merge with : ','rtmedia');
     				_e('Select Item to merge with : ','rtmedia');
     			 ?>
...
function rtmedia_album_edit ($options) {
    ...
    if ( rtmedia_is_album_editable() || is_rt_admin() ) {
        $options[] = "<a href='edit/' class='rtmedia-edit' title='" . //!!! __( 'Edit Album', 'rtmedia' )
		//!!!!! __( 'Edit Memory', 'rtmedia' )
		__( 'Edit Item', 'rtmedia' )
        . "' ><i class='rtmicon-edit'></i>" . //!!!__('Edit Album') 
        //!!!!! __('Edit Memory')
        __('Edit Item')
        . "</a>";
        $options[] = '<form method="post" class="album-delete-form rtmedia-inline" action="delete/">'
                ...
                //!!!!! . '<button type="submit" name="album-delete" class="icon-button rtmedia-delete-album" title="' . __( 'Delete Memory', 'rtmedia' ) . '"><i class="rtmicon-trash-o"></i>Delete Memory</button></form>';
                . '<button type="submit" name="album-delete" class="icon-button rtmedia-delete-album" title="' . __( 'Delete Item', 'rtmedia' ) . '"><i class="rtmicon-trash-o"></i>Delete Item</button></form>';
        ...
        if ( $album_list ) {
            ...
            $options[] = '<span><a href="#rtmedia-merge" class="rtmedia-reveal-modal rtmedia-modal-link" title="' . __('Merge Album', 'rtmedia') . '"><i class="rtmicon-code-fork"></i>' . __('Merge Item','rtmedia') . '</a></span>';
...
function rtmedia_selected_sub_nav(){
	global $rtMediaNav;
    $rtMediaNav = new RTMediaNav();
    $rtMediaNav->get_selected_sub_nav ();
}
...
function rtmedia_admin_premium_page($page) {
    ...
    <div class="premium-page-container">
        ...
        <div class="columns large-10">
            <h2><?php //!!! _e( 'Album Privacy', 'rtmedia'); 
    		_e( 'Memory Privacy', 'rtmedia');
    		?></h2>
            <p><?php 
            //!!! _e( 'This will allow you to set album privacy while creating albums or change album privacy with editing albums too.', 'rtmedia'); 
            _e( 'This will allow you to set memory privacy while creating memories or change memory privacy with editing memories too.', 'rtmedia');
            ?></p>
        </div>
        </div>
        <div class="row">
        ...
        <div class="columns large-10">
            <h2><?php //!!!_e( 'Global Albums', 'rtmedia'); 
            _e( 'Global Memories', 'rtmedia');
            ?></h2>
            <p><?php //!!! _e( 'Multiple global albums can be created beforehand. One of these can be chosen as the default album.', 'rtmedia'); 
            _e( 'Multiple global memories can be created beforehand. One of these can be chosen as the default memory.', 'rtmedia');
            ?></p>
        </div>
        </div>
        <div class="row">
...
/* get cover button for administrater */
function rtmedia_cover_button(){
    echo get_rtmedia_cover_button();
}

function get_rtmedia_cover_button(){
    global $rtmedia_media;
    ...
    if ((is_site_admin() && $pagename == 'drems') || (
            is_site_admin() && isset($_REQUEST['origin_page']) && $_REQUEST['origin_page'] == 'drems'))
       $cover_flag = true;
    ...
    $cover_flag = false;
    if (isset($rtmedia_media->cover_art) && $rtmedia_media->cover_art > 0){
        $cover_flag = true;
    }else{
        $cover_flag = false;
    }
    $btn_id = 'cover-media-'.rtmedia_id();
    if ($cover_flag){
        $label = 'Cover';
        $title = 'Not cover this item';
        $class = 'cover';
    }else{
        $label = 'Not Cover';
        $title = 'Cover this item';
        $class = 'no-cover';
    }
    $btn_htm = '<a href="#" class="button '.$class.' '.$btn_id.'" id="'.$btn_id.'" title="'.$title.'" onclick="change_cover('.rtmedia_id().'); return false;">'.$label.'</a>';
    return $btn_htm;
}