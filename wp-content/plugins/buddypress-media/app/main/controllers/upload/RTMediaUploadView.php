	...
	$album = '';
        if ( $rtmedia_query && isset( $rtmedia_query->media_query ) && isset( $rtmedia_query->media_query[ 'album_id' ] ) && is_rtmedia_album ()  ) {
            ...
            if ( isset( $rtmedia_query->query[ 'context' ] ) && $rtmedia_query->query[ 'context' ] == 'profile' ) {
                ...
                //!!!!! $album = '<span> <label> <i class="rtmicon-picture-o"></i>' . __('Memory ','rtmedia') . ': </label><select name="album" class="rtmedia-user-album-list">' . rtmedia_user_album_list () . '</select></span>';
                $album = '<span> <label> <i class="rtmicon-picture-o"></i>' . __(' To ','rtmedia') . ': </label><select name="album" class="rtmedia-user-album-list">' . rtmedia_user_album_list () . '</select></span>';
            }
            if (isset( $rtmedia_query->query[ 'context' ] ) && $rtmedia_query->query[ 'context' ] == 'group' ) {
                ...
                $album = '<span> <label> <i class="rtmicon-picture-o"></i>' . __('Memory ','rtmedia') . ': </label><select name="album" class="rtmedia-user-album-list">' . rtmedia_group_album_list () . '</select></span>';
            }
        }
                ...
                'default' => array( 'title' => __( 'File Upload', 'rtmedia' ),
                    ...
                    '<div id="rtmedia-upload-container" >'
                        . '<div id="drag-drop-area" class="drag-drop row">'
                                ...
                                . '<span class="rtm-seperator">' . __('or','rtmedia') .'</span><span class="drag-drop-info">' . __('Drop your files here', 'rtmedia') . '</span> <i class="rtm-file-size-limit rtmicon-info-circle"></i><a target="_blank" href="' .get_option('siteurl') .'/how-to/"> How To </a></div>'
                                . '<input type="button" class="start-media-upload" value="' . __('Start upload', 'rtmedia') .'" style="color: white!important; padding-top: 0px;"/>'
                        . '</div>'
                        . '<div class="row">'
