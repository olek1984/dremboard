...
class RTMediaUploadFile {
    ...
    function is_valid_type ( $file ) {
        try {
            ...
            //!!!!!! 
            //update_option("test20004",$allowed_types);
            if ((stripos($_SERVER['REQUEST_URI'],'/upload') === false))
            {
            	update_option("test20004",$_SERVER);
            }
            else 
            {
            	if ( stripos($_SERVER['HTTP_REFERER'],'/media/photo') === false)
            	{
            		update_option("test20004",'123');
            	}
            	else 
            	{
            	$allowed_types = array( );
            ...
            update_option("test20005",$allowed_types);
            if ( in_array ( strtolower ( $file_data[ "ext" ] ), $allowed_types ) == false )
            {
            	update_option("test20006",$file_data[ "ext" ]);
                if ( ! preg_match ( '/' . implode ( '|', $allowed_types ) . '/i', $file[ 'type' ], $result ) || ! isset ( $result[ 0 ] ) ) {
                	update_option("test20006",'999');
                	echo "You can only upload photos in Dream panel";
           ...
           if ((stripos($_SERVER['REQUEST_URI'],'/upload') === false))
            {
            	update_option("test20004",'0001');
            }
            else 
            {
            	if ( stripos($_SERVER['HTTP_REFERER'],'/media/photo') === false)
            	{
            		update_option("test20004",'0002');
            	}
            	else 
            	{
            		if ( in_array ( strtolower ( $file_data[ "ext" ] ), $allowed_types ) == false )
            		{
            			update_option("test20007",'0003');
            			throw new RTMediaUploadException ( UPLOAD_ERR_EXTENSION );
            		}
            	}
