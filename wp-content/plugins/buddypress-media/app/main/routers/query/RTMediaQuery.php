    ...
    function set_media_type () {
        if ( ! isset ( $this->query[ 'media_type' ] ) ) {
            if ( isset ( $this->action_query->id ) ) {
                $media = $this->model->get ( array( 'id' => $this->action_query->id, 'ps_flag' => false ) );
		if ( sizeof ( $media ) > 0 ) {
...
//!!!!!
    ...
    function is_dremboard_gallery () {
        if ( isset ( $this->action_query->media_type ) && $this->action_query->media_type == 'dremboard' ) {
            return true;
        }
        return false;
    }
...
//        }
    ...
    function &query ( $query ) {
        ...
        //var_dump($this->query);
        //!!!!!
        //[rtmedia_gallery global="true" media_type="album"]
...
//		    unset ( $this->query[ "album_id" ] );
        ...
        if ($is_dremboard || $is_album)
        {
        	//var_dump("4018");
        	$dremboard_result = array();
        	$filter_result = $this->get_data ();
        	//media_id
        	if ((is_array($filter_result)) && (count((is_array($filter_result))) > 0))
        	{
        		foreach ($filter_result as $results_now )
        		{
        	//var_dump("2012");
        	...
        	if ((isset($now_catarray)) && (is_array($now_catarray)) && (count($now_catarray) > 0))
        	{
        		...
        		*/
        		...
        		//var_dump($parent_cat);
        		$flag_cat = 0;
    			if ($is_dremboard) $flag_cat = 6;
    			if ($is_album) $flag_cat = 5;
    	        		
        		if ($parent_cat->parent == $flag_cat) //!!!!! 
...
//var_dump($rtmedia_query->is_gallery_shortcode);
        ...
        if ( isset ( $this->media_query[ 'context' ] ) ) {
            ...
            if ( $this->media_query[ 'context' ] == 'profile' ) {
                ...
                if (( ! $this->is_album_gallery () ) && ( ! $this->is_dremboard_gallery () ))
	...
	//!!!!!
//var_dump("1088");
//var_dump($this->media_query);
	$this->media_query = apply_filters( 'rtmedia_media_query', $this->media_query );

        //!!!!! if ( $this->is_album_gallery () ) {
        //!!!!!! for show dremboard shortcode if (( $this->is_album_gallery () ) || ( $this->is_dremboard_gallery () ) ){
        //!!!!!
        ...
        if ($shortcode_attr_type == 'album') $shortcode_for_album = true;
        
        if (( $this->is_album_gallery () ) || ( $this->is_dremboard_gallery () ) || ($rtmedia_query->is_gallery_shortcode && $shortcode_for_dremboard)  || ($rtmedia_query->is_gallery_shortcode && $shortcode_for_album))
        {
//var_dump("1089");
            if ( isset ( $author ) ) {
            	//!!!!! $query_function = 'get_user_albums'; //!!! in rtmediamodel.php
            	if ($this->is_dremboard_gallery ())
            	{
            		$query_function = 'get_user_dremboards'; //!!! in rtmediamodel.php
            	}
            ...
            {
            	$query_function = 'get_user_dremboards';
            }
            //!!!!!
            if (($rtmedia_query->is_gallery_shortcode && $shortcode_for_album))
            {
            ...
            //var_dump($query_function);
            //var_dump($media_for_total_count);
        } else {
        	//var_dump("2008");
            /**
...
//    }
    ...
    function album_or_media () {
        ...
        //!!!!!
//var_dump("1091");
//var_dump($this->action_query->media_type);
//var_dump($allowed_media_types);
        if ( ! isset ( $this->media_query[ 'media_type' ] ) ) {
        	/* //!!!!! in here it will be $this->action_query->media_type == 'album'
            ...
            */
            if ( isset ( $this->action_query->media_type ) &&
                    (
                    in_array ( $this->action_query->media_type, $allowed_media_types ) ||
                    ($this->action_query->media_type == 'album') || ($this->action_query->media_type == 'dremboard')
                    )
            ) {
               //!!!!! $this->media_query[ 'media_type' ] = $this->action_query->media_type;
               if (($this->action_query->media_type == 'dremboard'))
               {
               		$this->media_query[ 'media_type' ] = 'album';
               }
               else 
               {
               		$this->media_query[ 'media_type' ] = $this->action_query->media_type;
    ...
    function populate_data () {
        ...
        if ( $this->action_query->action == 'comments' && ! isset ( $this->action_query->id ) )
            ...
            //!!!!!$this->media = $this->populate_media ();
            //var_dump("4080");
//var_dump($this->shortcode_global);
//var_dump("4072");
//var_dump($this->media);
        if ( empty ( $this->media ) )
            return;
    ...
    function have_media () {
		//!!!
		if (function_exists('bp_privacy')) 
		{
			//var_dump("2001");
			$bp_privacy = bp_privacy();
		...
		//var_dump("2002");
        $total = $this->media_count; //!!!!! this is count
        ...
        if ( $curr + 1 < $per_page && $total > $offset + $curr + 1 ) {
        	...
        	//var_dump("2004");
        	//!!!!! var_dump("1063"); this will show load more
...
//var_dump("2005");
    ...
    function rtmedia () {
        ...
        if ( $this->current_media == -1 ) // loop has just started
            do_action_ref_array ( 'rtmedia_loop_start', array( &$this ) ); //!!!!! --- this will be run,but no effect
//!!!!! in here $rtmedia_media get all posts
