...
//		return false;
    ...
    if ($trend_flag){
        $activity_action  = sprintf( __( '%s posted a trend drēm', 'buddypress' ), $from_user_link );
        $component    = 'trend';
    }
...
function bp_activity_get_permalink( $activity_id, $activity_obj = false ) {
	...
	if ( 'new_blog_post' == $activity_obj->type || 'new_blog_comment' == $activity_obj->type || 'new_forum_topic' == $activity_obj->type || 'new_forum_post' == $activity_obj->type ) {
		...
		if ( 'activity_comment' == $activity_obj->type ) {
			$link = bp_get_root_domain() . '/' . bp_get_activity_root_slug() . '/' . $activity_obj->item_id . '/';
		} else {
			$link = bp_get_root_domain() . '/' . bp_get_activity_root_slug() . '/' . $activity_obj->id . '/';
		}
	}
...
function bp_get_media_by_activity($activity_id, $media_type=false){
    $model = new RTMediaModel();
    if($media_type){
        $media = $model->get_media ( array( 'activity_id' => $activity_id, 'media_type' => $media_type, 'ps_flag' => false ), false, false );
    }else{
        $media = $model->get_media ( array( 'activity_id' => $activity_id, 'ps_flag' => false ), false, false );
    }
...
/**
 * Set the flag for this activity
 *
 ...
 */
function bp_set_user_activity_flag( $activity_id = 0, $user_id = 0, $flag = '' ) {
	if ( empty( $activity_id ) || empty( $user_id ) || empty( $flag ) ) {
		return false;
	}

	// Stored in an array in usermeta
	$current_activity_flags = bp_get_user_meta( $user_id, 'bp_activity_flags', true );

	if ( !$current_activity_flags ) {
		$current_activity_flags = array();
	}

	$current_activity_flags[$activity_id] = $flag;

	return bp_update_user_meta( $user_id, 'bp_activity_flags', $current_activity_flags );
}

function bp_check_dcma_dispute_counter_data($data, &$msg = false){
    $ret_val = true;    
    // check all requested data is supported.
    if( !isset($data['owner_name']) || !isset($data['owner_email']) || !isset($data['case_id']) || 
        !isset($data['reported_content_url'])){
        $ret_val = false;
        $msg = 'Please make sure all requested data';
        return $ret_val;
    }
    
    // check agent's inform
    $owner_id = bp_loggedin_user_id();
    $owner = bp_core_get_core_userdata($owner_id);
    if ($owner->user_email != $data['owner_email']){
        $ret_val = false;
        $msg = 'Your Email address is not the same as one registed in site.';
        return $ret_val;
    }
    
    // check reported_content_url is correct
    $reported_url_start = home_url(bp_get_activity_root_slug() .'/');
    foreach ($data['reported_content_url'] as $key => $reported_content_url) {
        $case_id = $data['case_id'][$key];
        
        if (!filter_var($reported_content_url, FILTER_VALIDATE_URL)){
            $ret_val = false;
            $msg = 'You reported content url('.$reported_content_url.') is not vailed.';
            return $ret_val;
        }
        
        // check reported_content_url has correct style.
        $activity_id = strtr($reported_content_url, array($reported_url_start => ""));
        if (!is_numeric($activity_id)){
            $ret_val = false;
            $msg = 'You reported content url('.$reported_content_url.') is not vailed.';
            return $ret_val;
        ...
        // check reported user is correct.
        
        $at_dcma = new BP_Activity_Dcma($case_id);
        
        if (!$at_dcma->activity_id){
            $ret_val = false;
            $msg = 'You reported case id(#case:'.$case_id.') is not exist.';
            return $ret_val;
        }
        
        if ($at_dcma->owner_id != $owner_id){
            $ret_val = false;
            $msg = 'You are not a owner for #case:'.$case_id.".";
            return $ret_val;
        }
        
        if ($at_dcma->activity_id != $activity_id){
            $ret_val = false;
            $msg = 'You reported content url('.$reported_content_url.') is not mensioned in #case:'.$case_id.".";
            return $ret_val;
        }
        
        if ($at_dcma->dmca_step != 'counter_claim'){
            $ret_val = false;
            $msg = 'You reported content('.$reported_content_url.') is already processed by DCMA';
            return $ret_val;
        }
    }
    return $ret_val;
...
function bp_create_dcma_dispute_counter($data) {
    $dmca_step = "created";
            
    foreach ($data['case_id'] as $key => $case_id) {
        $at_dcma = new BP_Activity_Dcma($case_id);
        
        switch ($data['reason'][$key]) {
            case 'mis_identify':
                $reason = '<p>This drēm does not feature the the third-party copyrighted material at issue. My drēm was misidentified as containing this material.</p>';
                break;
            case 'fair_use':
                $reason = '<p>This drēm uses copyrighted material in a manner that does not require approval of the copyright holder. It is a fair use under copyright law.</p>';
                $reason .= '<strong>Brief Explain: </strong>'.$data['fair_use_brief'][$key];
                break;
            case 'appropriate':
                $reason = '<p>This drēm uses the copyrighted material at issue, but with the appropriate authorization from the copyright owner.</p>';
                $reason .= '<strong>Brief Explain: </strong>'.$data['appropriate_brief'][$key];
                break;
            default:
                break;
        }
        $at_dcma->update_dispute_counter_reason($reason);
        $at_dcma->update_step ($dmca_step);
        $at_dcma->dmca_step4(); // send email and remove drems really.
    }
}
///***** functions for check dcma counter claim values ******///
function bp_check_dcma_counter_claim_data($data, &$msg = false){
    $ret_val = true;
    
    // check all requested data is supported.
    if( !isset($data['agent_name']) || !isset($data['agent_email']) || !isset($data['case_id']) || 
        !isset($data['reported_content_url'])){
        $ret_val = false;
        $msg = 'Please make sure all requested data';
        return $ret_val;
    ...
    // check agent's inform
    $agent_id = bp_loggedin_user_id();
    $agent = bp_core_get_core_userdata($agent_id);
    if ($agent->user_email != $data['agent_email']){
        $ret_val = false;
        $msg = 'Your Email address is not the same as one registed in site.';
        return $ret_val;
    }
    ...
    foreach ($data['reported_content_url'] as $key => $reported_content_url) {
        ...
        if ($at_dcma->agent_id != $agent_id){
            $ret_val = false;
            $msg = 'You are not a agent for #case:'.$case_id.".";
            return $ret_val;
        }
...
function bp_create_dcma_counter_claim($data){
    $dmca_step = 'counter_claim';
    ...
    foreach ($data['case_id'] as $key => $case_id) {
        ...
        $at_dcma->update_counter_claim_reason($reason);
        $at_dcma->update_step ($dmca_step);
        $at_dcma->dmca_step3(); // change step as 'counter_claim' and email to dremboard, owner, agent.
    }
}

///***** functions for check dcma requested values ******///
function bp_check_dcma_request_data($data, &$msg = false){
    $ret_val = true;
    
    // check all requested data is supported.
    if( !isset($data['agent_name']) || !isset($data['agent_email']) || 
        !isset($data['owner_name']) || !isset($data['owner_email']) || 
        !isset($data['owner_website_url']) || !isset($data['reported_content_url']) || 
        !(isset($data['is_accurate']) && $data['is_accurate'] == '1') || 
        !(isset($data['is_good_faith']) && $data['is_good_faith'] == '1') || 
        !(isset($data['is_authorized_agent']) && $data['is_authorized_agent'] == '1')){
        $ret_val = false;
        $msg = 'Please make sure all requested data';
        return $ret_val;
    }
    
    // check owner's inform
    $owner_id = bp_loggedin_user_id();
    $owner = bp_core_get_core_userdata($owner_id);
    if ($owner->user_email != $data['owner_email']){
        $ret_val = false;
    ...
    // check owner_website url, reported_content_url is correct
    $reported_url_start = home_url(bp_get_activity_root_slug() .'/');
    foreach ($data['owner_website_url'] as $key => $owner_website_url) {
        $reported_content_url = $data['reported_content_url'][$key];
        if (!filter_var($owner_website_url, FILTER_VALIDATE_URL)){
            $ret_val = false;
            $msg = 'Your owner website url('.$owner_website_url.') is not vailed.';
            return $ret_val;
        }
        if (!filter_var($reported_content_url, FILTER_VALIDATE_URL)){
            $ret_val = false;
        ...
        if ($agent->user_email != $data['agent_email']){
            $ret_val = false;
            $msg = 'You reported content('.$reported_content_url.') is not '.$data['agent_name'].' posted, or the content is not exist.';
            return $ret_val;
        }
        
        $at_dcma = new BP_Activity_Dcma();
        $at_status = $at_dcma->checkActivity($activity_id);

        if ($at_status == 'is_processing' || $at_status == 'removed'){
            $ret_val = false;
            $msg = 'You reported content('.$reported_content_url.') is already processed by DCMA';
            return $ret_val;
        }
...
function bp_create_dcma_request ($data, $agent_id){
    $case_ids = array();
    $owner_id = bp_loggedin_user_id();
    $agent_id = $agent_id;
    $is_accurate = $data['is_accurate'];
    $is_good_faith = $data['is_good_faith'];
    $is_authorized_agent = $data['is_authorized_agent'];
    $dmca_step = 'created';
    $date = new DateTime();
    ...
    foreach ($data['owner_website_url'] as $key => $owner_website_url) {
        $reported_content_url = $data['reported_content_url'][$key];
        $remove_all = $data['remove_all'][$key];
        $strike = $data['strike'][$key];
        $activity_id = strtr($reported_content_url, array($reported_url_start => ""));
        
        $at_dcma = new BP_Activity_Dcma();
        $at_dcma->owner_id             = $owner_id;
        $at_dcma->agent_id             = $agent_id;
        $at_dcma->owner_website_url    = $owner_website_url;
        $at_dcma->reported_content_url = $reported_content_url;
        $at_dcma->activity_id          = $activity_id;
        $at_dcma->remove_all           = $remove_all;
        $at_dcma->strike               = $strike;
        $at_dcma->is_accurate          = $is_accurate;
        $at_dcma->is_good_faith        = $is_good_faith;
        $at_dcma->is_authorized_agent  = $is_authorized_agent;
        $at_dcma->dmca_step            = $dmca_step;
        $at_dcma->update_date          = $update_date;
        $at_dcma->save();
        $case_ids[] = $at_dcma->case_id;
        $at_dcma->dmca_step1(); // send Email to Dremboard, agent, owner.
        $at_dcma->dmca_step2(); // remove activity from drembaord as flag.
    }
    return $case_ids;
}