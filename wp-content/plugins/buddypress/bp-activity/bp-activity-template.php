...
class BP_Activity_Template {
    ...
    function __construct($args) {
        ...
        if (!empty($include))
            $this->activities = bp_activity_get_specific(array('activity_ids' => explode(',', $include), 'max' => $max, 'page' => $this->pag_page, 'per_page' => $this->pag_num, 'sort' => $sort, 'display_comments' => $display_comments, 'show_hidden' => $show_hidden, 'spam' => $spam, 'nt_flag' => $nt_flag, 'scope' => $scope,'last_activity_id' => $last_activity_id));

        // Fetch all activity items
        else
            $this->activities = bp_activity_get(array('display_comments' => $display_comments, 'max' => $max, 'per_page' => $this->pag_num, 'page' => $this->pag_page, 'sort' => $sort, 'search_terms' => $search_terms, 'meta_query' => $meta_query, 'filter' => $filter, 'show_hidden' => $show_hidden, 'exclude' => $exclude, 'in' => $in, 'spam' => $spam, 'nt_flag' => $nt_flag, 'scope' => $scope,'last_activity_id' => $last_activity_id));
...
function bp_has_activities($args = '') {
    ...
    // exclude for login user's flaged activities.
    ...
    if ('just-me' == $scope || 'following' == $scope || 'friends' == $scope || 'groups' == $scope || 'favorites' == $scope || 'mentions' == $scope || 'notifications' == $scope) {

        if ('just-me' == $scope || 'notifications' == $scope)
...
function bp_get_activity_secondary_avatar($args = '') {
    ...
    switch ($activities_template->activity->type) {
        case 'activity_own' :
    ...
    if (!empty($linked)) {
        ...
        return sprintf('<a href="%s" class="%s">%s</a>', $link, $link_class, apply_filters('bp_get_activity_secondary_avatar', $avatar)
        );
    }
...
function bp_get_activity_content_body() {
    ...
    require_once( BP_PLUGIN_DIR . '/emojione/lib/php/autoload.php' );
    $activities_template->activity->content = Emojione\Emojione::shortnameToImage($activities_template->activity->content);
...
function bp_get_activity_latest_update($user_id = 0) {
    ...
    require_once( BP_PLUGIN_DIR . '/emojione/lib/php/autoload.php' );
    $update_content = Emojione\Emojione::shortnameToImage($update['content']);

    //$latest_update = apply_filters('bp_get_activity_latest_update_excerpt', trim(strip_tags(bp_create_excerpt($update['content'], 358))));
    $latest_update = apply_filters('bp_get_activity_latest_update_excerpt', trim(bp_create_excerpt($update_content, 358)));
    $latest_update .= ' <a href="' . bp_get_root_domain() . '/' . bp_get_activity_root_slug() . '/' . $update['id'] . '/"> ' . __('View', 'buddypress') . '</a>';
