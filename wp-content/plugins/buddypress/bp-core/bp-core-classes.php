...
class BP_User_Query {
	...
	public function prepare_user_ids_query() {
        ...
        // this part is for members widget.
        $special_user_only = false;
        switch ( $type ) {
            case 'family':
                $special_user_only = 'family';
                $type = 'active';
                break;
            case 'friend':
                $special_user_only = 'friend';
                $type = 'active';
                break;
            case 'following':
                $special_user_only = 'following';
                $type = 'active';
                break;
            case 'follower':
                $special_user_only = 'follower';
                $type = 'active';
                break;
            case 'friendship_request':
                $special_user_only = 'friendship_request';
                $type = 'active';
                break;
            case 'familyship_request':
                $special_user_only = 'familyship_request';
                $type = 'active';
                break;
            case 'blocked':
                $special_user_only = 'blocked';
                $type = 'active';
                break;
        }
		...
		// @todo remove need for bp_is_active() check
		if (( $special_user_only == false && ! empty( $user_id ) && bp_is_active( 'friends' ) )
                || ($special_user_only == 'friends' && bp_is_active( 'friends' ))) {
        ...
        if ($special_user_only == 'family' && bp_is_active( 'familys' )) {
			$family_ids = familys_get_family_user_ids( $user_id );
			$family_ids = implode( ',', wp_parse_id_list( $family_ids ) );

			if ( ! empty( $family_ids ) ) {
				$sql['where'][] = "u.{$this->uid_name} IN ({$family_ids})";

			// If the user has no familys, the query should always
			// return no users
			} else {
				$sql['where'][] = $this->no_results['where'];
			}
        ...
        if ($special_user_only == 'following'){
            $following_ids = bp_follow_get_following(array('user_id' => $user_id));
            $following_ids = implode( ',', wp_parse_id_list( $following_ids ) );

			if ( ! empty( $following_ids ) ) {
				$sql['where'][] = "u.{$this->uid_name} IN ({$following_ids})";

			// If the user has no following, the query should always
			// return no users
			} else {
				$sql['where'][] = $this->no_results['where'];
			}
        }
        if ($special_user_only == 'follower'){
            $followers_ids = bp_follow_get_followers(array('user_id' => $user_id));
            $followers_ids = implode( ',', wp_parse_id_list( $followers_ids ) );

			if ( ! empty( $followers_ids ) ) {
				$sql['where'][] = "u.{$this->uid_name} IN ({$followers_ids})";

			// If the user has no followers, the query should always
			// return no users
			} else {
				$sql['where'][] = $this->no_results['where'];
			}
        }
        if ($special_user_only == 'blocked'){
            $blocked_ids = bpb_get_blocked_users_list_by_type( array('user_id' => $user_id) );
            $blocked_ids = implode( ',', $blocked_ids );
            $blocked_ids = implode( ',', wp_parse_id_list( $blocked_ids ) );
			if ( ! empty( $blocked_ids ) ) {
				$sql['where'][] = "u.{$this->uid_name} IN ({$blocked_ids})";
        ...
        if ($special_user_only == 'friendship_request'){
            $friendship_request_ids = friends_get_friend_user_ids($user_id, true);
            $friendship_request_ids = implode( ',', wp_parse_id_list( $friendship_request_ids ) );

			if ( ! empty( $friendship_request_ids ) ) {
				$sql['where'][] = "u.{$this->uid_name} IN ({$friendship_request_ids})";
        ...
        if ($special_user_only == 'familyship_request'){
            $familyship_request_ids = familys_get_family_user_ids($user_id, true);
            $familyship_request_ids = implode( ',', wp_parse_id_list( $familyship_request_ids ) );

			if ( ! empty( $familyship_request_ids ) ) {
				$sql['where'][] = "u.{$this->uid_name} IN ({$familyship_request_ids})";
...
public static function search_users_by_name( $search_terms, $limit = null, $friends = false, $page = 1, $populate_extras = true ) {
		global $bp, $wpdb;
		...
		if ($friends == "") $friends = "-1";
			$friends_where = " u.ID IN ({$friends}) AND ";
		}
		...
		$total_users_sql = apply_filters( 'bp_core_search_users_count_sql', "SELECT COUNT(DISTINCT u.ID) as id FROM {$wpdb->users} u LEFT JOIN {$bp->profile->table_name_data} pd ON u.ID = pd.user_id WHERE {$friends_where} {$status_sql} AND ( ( pd.id in ( SELECT pd.id FROM {$bp->profile->table_name_fields} pf, {$bp->profile->table_name_data} pd WHERE pf.id = pd.field_id AND pf.name = Name AND pd.value LIKE '%%{$search_terms}%%' )) OR u.user_nicename LIKE '%%{$search_terms}%%' ) ORDER BY pd.value ASC", $search_terms );
		$paged_users_sql = apply_filters( 'bp_core_search_users_sql',       "SELECT DISTINCT u.ID as id, u.user_registered, u.user_nicename, u.user_login, u.user_email FROM {$wpdb->users} u LEFT JOIN {$bp->profile->table_name_data} pd ON u.ID = pd.user_id WHERE {$friends_where} {$status_sql} AND ( ( pd.id in ( SELECT pd.id FROM {$bp->profile->table_name_fields} pf, {$bp->profile->table_name_data} pd WHERE pf.id = pd.field_id AND pf.name = Name AND pd.value LIKE '%%{$search_terms}%%' )) OR u.user_nicename LIKE '%%{$search_terms}%%' ) ORDER BY pd.value ASC{$pag_sql}", $search_terms, $pag_sql );
