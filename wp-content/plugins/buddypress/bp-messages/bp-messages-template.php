...
class BP_Messages_Box_Template {
	...
	function __construct( $user_id, $box, $per_page, $max, $type, $search_terms, $page_arg = 'mpage', $page = false ) {
...
function bp_has_message_threads( $args = '' ) {
	...
	if ( bp_is_current_action( 'notices' ) && !bp_current_user_can( 'bp_moderate' ) ) {
		...
		$messages_template = new BP_Messages_Box_Template( $user_id, $box, $per_page, $max, $type, $search_terms, $page_arg, $page );
	}
...
function bp_the_thread_message_content() {
	...
	function bp_get_the_thread_message_content() {
		...
		$content =  bp_activity_at_name_filter($thread_template->message->message);
		if ("Shared message" != $thread_template->message->subject){
