...
//error_log(print_r($grouped_notifications, true));
	// Bail if no notification groups
	if ( empty( $grouped_notifications ) ) {
		return false;
	}
	...
	// Calculate a renderable output for each notification type
	foreach ( $grouped_notifications as $component_name => $action_arrays ) {
//error_log(print_r($action_arrays, true));
		// Skip if group is empty
		if ( empty( $action_arrays ) ) {
			continue;
		}
		...
		// Loop through each actionable item and try to map it to a component
		foreach ( (array) $action_arrays as $component_action_name => $component_action_items ) {
//error_log(print_r($component_action_name, true));
//error_log(print_r($component_action_items, true));
			foreach ( (array) $component_action_items as $notification_item ) {
                            
                            $notification_item_count = count( $notification_item );
                            // Skip if the count is less than 1
                            if ( $notification_item_count < 1 ) {
                                    continue;
                            }    
                            ...
                            if ( isset( $bp->{$component_name}->notification_callback ) && is_callable( $bp->{$component_name}->notification_callback ) ) {
                                    ...
                                    if ( 'object' === $format ) {
                                            ...
                                            $notification_object->id = $notification_item->id;
                                            $renderable[]            = $notification_object;

                                    // Return an array of content strings
                                    } else {
                                            $content      = call_user_func( $bp->{$component_name}->notification_callback, $component_action_name, $notification_item->item_id, $notification_item->secondary_item_id, $notification_item_count );
                                            $renderable[] = $content;
                                    }

                            // @deprecated format_notification_function - 1.5
                            } elseif ( isset( $bp->{$component_name}->format_notification_function ) && function_exists( $bp->{$component_name}->format_notification_function ) ) {
                                    $renderable[] = call_user_func( $bp->{$component_name}->format_notification_function, $component_action_name, $notification_item->item_id, $notification_item->secondary_item_id, $notification_item_count );
