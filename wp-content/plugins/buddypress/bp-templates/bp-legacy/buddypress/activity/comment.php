...
<li id="acomment-<?php bp_activity_comment_id(); ?>">
	...
	<div class="acomment-options">
		...
		<?php if ( is_user_logged_in() && bp_activity_can_comment_reply( bp_activity_current_comment() ) ) : ?>
			...
			<a href="#acomment-<?php bp_activity_comment_id(); ?>" class="acomment-reply bp-primary-action reply" id="acomment-reply-<?php bp_activity_id(); ?>-from-<?php bp_activity_comment_id(); ?>"><?php _e( 'Reply', 'buddypress' ); ?></a>

		<?php endif; ?>
		<?php
		/* translators: 1: user profile link, 2: user name, 3: activity permalink, 4: activity timestamp */
		printf( __( '<span style="font-size:12px;color:#000;margin-left:10px;">replied</span> <a href="%1$s" class="activity-time-since"><span class="time-since">%2$s</span></a>', 'buddypress' ), bp_get_activity_comment_permalink(), bp_get_activity_comment_date_recorded() );
		?>
