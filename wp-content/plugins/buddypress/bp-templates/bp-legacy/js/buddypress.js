...
function resizeCommentImg(file, file_view_img, file_view, input_file_box){
  var cvtMaxWidth = 600;
  var cvtMaxHeight = 380;
  var image = new Image();

  image.onload = function(){
      var width = image.width;
      var height = image.height;
      var scaleX = width / cvtMaxWidth;
      var scaleY = height / cvtMaxHeight;
      ...
      file_view_img.attr("src", canvas.toDataURL("image/png"));
      file_view.show();
	  input_file_box.hide();
  };
...
function init_comment_file_in_container(container){
	jq(".ac-input-file", container).change(function(e){
		var input_file = jq(this);
		var input_file_box = input_file.parent();
		...
		if (filesize > 3){
			alert ("File size is over then the max file size limit(3MB).");
			jq(this).val("");
			return;
		}
		
		var reader = new FileReader();
		var file_view = jq("#"+jq(this).attr("id").replace("ac-input-file-", "ac-file-view-"));
		var file_view_img = jq("img", file_view);
		
		resizeCommentImg(file, file_view_img, file_view, input_file_box)
		...
		reader.onload = function(e){
			...
			file_view_img.attr("src", e.target.result);
			file_view.show();
			input_file_box.hide();
		}
	...
	jq(".ac-file-view .close", container).click(function(){
		var file_view = jq(this).parent();
		var file_view_img = jq("img", file_view);
		var input_file = jq("#"+file_view.attr("id").replace("ac-file-view-", "ac-input-file-"));
		var input_file_box = input_file.parent();
		input_file.val("");
		file_view_img.attr("src","");
		file_view.hide();
		input_file_box.show();
		var content_body = jQuery(".rtm-single-meta-contents.logged-in");
		var comment_body = jQuery(".rtm-media-single-comments");
...
function init_comment_file(){
	jq(".ac-input-file").change(function(e){
		var input_file = jq(this);
		var input_file_box = input_file.parent();
		var files = e.currentTarget.files;
		var file = files[0];
	...
	jq(".ac-file-view .close").click(function(){
		var file_view = jq(this).parent();
		var file_view_img = jq("img", file_view);
		var input_file = jq("#"+file_view.attr("id").replace("ac-file-view-", "ac-input-file-"));
		var input_file_box = input_file.parent();
...
jq(document).ready( function() {
	...
	jq("#aw-whats-new-submit").on( 'click', function() {
		...
		//var content = jq("#whats-new").val();
                var content = emojione.toShort(jq("#whats-new").val());
		...
		function(response) {
			...
			if ( response[0] + response[1] == '-1' ) {
				...
				jq("#activity-stream li:first a.edit-activity").magnificPopup({type: 'ajax', mainClass: 'mfp-flex-width'});
				init_comment_file_in_container(jq("#activity-stream li:first .activity-comments form"));
				add_input_for_bp_user(jQuery("#activity-stream li:first .activity-comments form textarea"));
			...
			jq("#aw-whats-new-submit").prop("disabled", true).removeClass('loading');
                        ...
                        jq('#whats-new-post-in').val(0);
                        jq("#rtmedia-action-update .privacy").prop('disabled',false).val(0).show();
		});

		return false;
	});
	jq('.mobile_sub_menu ul.has_sub').on( 'click', function(event) {
			var actionBtn = jq(this);
			var sub_menu = actionBtn.next();
			if(sub_menu.hasClass("sub") == false)
				return false;
			if (actionBtn.hasClass("open")){
				sub_menu.slideUp("slow");
				actionBtn.removeClass("open");
			}else{
				actionBtn.addClass("open");
	...
	jq('div.activity').on( 'click', function(event) {
		...
		if ( target.hasClass('acomment-reply') || target.parent().hasClass('acomment-reply') ) {
			...
			var comments = target.closest(".activity-item").find(".activity-comments");
			if(comments.length != 0 && comments.css("display") == "none"){
				comments.show();
			}
		...
		if ( target.attr('name') == 'ac_form_submit' ) {
			...
			/*if(confirm("Want to check comments?")){
				jq.post( ajaxurl, {
					action: 'get_single_activity_comments',
					'activity_id': comment_id
				},
				function(response) {
					var activity_comments = form.parent();
					var new_comments = jQuery(response);
						activity_comments.find("ul:first").remove();
						activity_comments.prepend(new_comments);
				});
				return false;
			}*/
			...
			var file_data = jq(".ac-input-file", form).prop("files")[0];
			
			form_data.append("action", 'new_activity_comment');
			form_data.append('cookie', bp_get_cookies());
			form_data.append('_wpnonce_new_activity_comment', jq("#_wpnonce_new_activity_comment").val());
			form_data.append('comment_id', comment_id);
			form_data.append('form_id', form_id[2]);
			form_data.append('content', content_data);
			
			var img_url = jq(".ac-input-file", form).val();
			if (img_url != null && img_url != ""){
				form_data.append('file_name', jq(".ac-input-file", form).prop("files")[0].name);
			}
			
			// Akismet
			var ak_nonce = jq('#_bp_as_nonce_' + comment_id).val();
			if ( ak_nonce ) {
				form_data.append('_bp_as_nonce_' + comment_id, ak_nonce);
			}
			...
			jQuery.ajax({
			    ...
			    success:function(response){
					...
					response = jq.trim(response);
					/* Check for errors and append if found. */
					if ( response[0] + response[1] == '-1' ) {
						form.append( jq( response.substr( 2, response.length ) ).hide().fadeIn( 200 ).click(function(){jq(this).fadeOut( 2000 );}) );
					} else {
						var activity_comments = form.parent();
						form.fadeOut( 200, function() {
							...
							if(activity_comments.css("display") == "none"){
								activity_comments.show();
							}
						} );
						jq( '#' + form.attr('id') + ' textarea').val('');
						jq(".ac-file-view img", form).attr("src", "");
						jq(".ac-input-file", form).val("");
						jq(".ac-file-view", form).hide();
						jq(".ac-file", form).show();
	...
	jq('#buddypress').on( 'click', function(event) {
		...
		if ( target.hasClass('button') )
			return true;
                ...
                var media_item = target.closest(".rtmedia-item, .rtmedia-item-thumbnail");
                var media_content = target.closest(".activity-content.mobile");
                if(media_item.length != 0 && media_content.length != 0){
                    var link = media_content.find(".mobile_link").val();
    ...
    auto_refresh_home_page();
    auto_load_more();
    init_notification_inform();
    preloadAllVideo();
    jQuery("#gk-user-chat").click(function(){
        var chat_btn = jQuery(".iflychat-mob-btn");
        if(chat_btn.length == 0){
            alert("Chat is not ready.");
        }else{
            window.open(chat_btn.attr("href"));
        }
    });
    
    jQuery("a.edit-activity").magnificPopup({type: 'ajax', mainClass: 'mfp-flex-width'});
});

function auto_load_more(){
    if(jQuery(".load-more-hidden:not(.loading)").length == 0)
        return;
...
function load_more_action() {
    if (is_load_more_shown()) {
        console.log("load more action is ..");
        var load_more = jQuery(".load-more-hidden:not(.loading)");
...
function auto_refresh_home_page(){
    if(jQuery("body.home-page").length == 0){
        return;
    }

    var timer = null; //setInterval(refresh, 10 * 60000);
    ...
    jQuery(document.body).bind("click keypress", function(e) {
        restartTimer();
    });
    ...
    function refresh() {
        if (refresh_flag){
//            if (playing_state_flag !== true)
//            {
                jQuery(".item-list-tabs ul li.selected a").click();
                //window.location.href = jQuery(".item-list-tabs ul li.selected a").attr('href');
                //jQuery("#gk-top ul li.current-menu-item a").click();
//            }
            restartTimer();
        }else{
            clearTimeout(timer);
    ...
    function restartTimer(){
        if (timer == null){
            timer = setTimeout(refresh, 3 * 60000);
        }else{
            clearTimeout(timer);
            timer = setTimeout(refresh, 3 * 60000);
        }
...
function start_highlight(target, high_color){
	var high_id;
	var target_item = target;
	var highlight_color = high_color;
	if (target.hasClass("highlight"))
		return;
	target_item.addClass("highlight");
	high_id = setInterval(function(){
		if(target_item.hasClass("highlight")){
			if (highlight_color !== undefined){
				target_item.effect("highlight", { color: highlight_color }, 2000);
			}else{
				target_item.effect("highlight", { color: "#ffffff" }, 2000);
			}
		}else{
			clearInterval(high_id);
		}
	}, 3000);
	target_item.click(function(){
		if(jQuery(this).closest('ul').prev().hasClass("highlight")){
			jQuery(this).closest('ul').prev().removeClass("highlight")
		}
		jQuery(this).removeClass("highlight");
	});
}

function stop_highlight(target){
	target.removeClass("highlight");
}

var preloadAllVideo = function() {
	setTimeout(function(){
		jQuery("video[preload='none']").each( function(){
			jQuery(this).attr("preload", "true");
    	});
	}, 10000);
}

function init_edit_activity_submit(){
    jq("#save_activity_changes").click(function(e){
        var $form = jq("#activity-edit-form");
        var formdata = $form.serialize();
        formdata += (formdata!=='')? '&':'';
        formdata += 'save_changes=true&ajax_flag=true';
        ...
        jQuery.ajax({
            ...
            success: function(data) {
                var ori_activity_id = jq("#origin_activity").val();
                var $activity_text = jq("#"+ori_activity_id+" .rtmedia-activity-text");
                var $activity_container = jq("#"+ori_activity_id+" .rtmedia-activity-container");
                if($activity_text.length == 0){
                    if($activity_container.length == 0){
                        jq("#"+ori_activity_id+" .activity-inner").html("<p>"+jq("#activity_content").val()+"</p>");
                    }else{
                        $activity_container.prepend("<div class='rtmedia-activity-text'>"+jq("#activity_content").val()+"</div>");
                    }
                }else{
                    $activity_text.html(jq("#activity_content").val());
                }
                jq(".mfp-close").click();
            }
        });
...
function get_notification_inform(){
	...
	function(response) {
		...
		var notification_dump = Array();
		var showed_nt = Array();
		var showed_nt_dump = Array();
		var i;
		notifications = Array();
		result = result.replace(/[^\}\]]*$/, "");
		
		if(result.length !== 0){
			// get the showed notification ids from localStorage.
			
			var showed_nt = JSON.parse(localStorage.getItem('nt_ids'));
			notification_dump = JSON.parse(result);
			
			if (showed_nt !== null && showed_nt.length !== 0){
				for(i = 0; i < notification_dump.length; i++){
					if (jQuery.inArray(notification_dump[i].id, showed_nt) != -1){
						// it is already showed.
						showed_nt_dump.push(notification_dump[i].id);
					}else{
						// it have to be showed.
						notifications.push(notification_dump[i]);
					}
				}
			}else{
				notifications = notification_dump;
			}
			
			if (jQuery("body.home-page").length != 0){
				if( notifications.length > 0 ){
					start_highlight(jQuery(".activity-type-tabs.mobile .has_sub"));
					start_highlight(jQuery(".activity-type-tabs.mobile #activity-notifications"), "#969696");
				}else{
					stop_highlight(jQuery(".activity-type-tabs.mobile .has_sub"));
					stop_highlight(jQuery(".activity-type-tabs.mobile #activity-notifications"));
				}
			}
		}
		localStorage.setItem('nt_ids', JSON.stringify(showed_nt_dump));
	});
}
...
function bp_activity_set_scope(scope) {
    var filter = -1;
	/* Save the type and filter to a session cookie */
	jq.cookie( 'bp-activity-scope', scope, {
		path: '/'
