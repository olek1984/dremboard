...
function invite_anyone_screen_one_content() {
	...
	if ( !empty( $iaoptions['email_limit_invites_toggle'] ) && $iaoptions['email_limit_invites_toggle'] == 'yes' && !current_user_can( 'delete_others_pages' ) ) {
		...
		if ( $sent_invites_count >= $iaoptions['limit_invites_per_user'] ) : ?>
			...
			<h4 style="margin-left: 25px;"><?php _e( 'Invite New Members', 'bp-invite-anyone' ); ?></h4>

			<p id="welcome-message" style="margin-left:25px;"><?php _e( 'You have sent the maximum allowed number of invitations.', 'bp-invite-anyone' ); ?></em></p>
  ...
  ?>
	...
	<p id="welcome-message" style="margin-left: 25px;"><?php echo $welcome_message ?></p>
