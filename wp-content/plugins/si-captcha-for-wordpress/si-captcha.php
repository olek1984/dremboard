...
//  echo '<br />
  echo '<input id="captcha_code" name="captcha_code" class="input" type="text" value="" size="12" tabindex="30" '.$si_aria_required.' style="font-size: 24px; width: 97%; padding: 3px; margin-top: 2px; margin-right: 6px; margin-bottom: 16px; border: 1px solid #e5e5e5; background: #fbfbfb;" /></label>
</p>
...
function si_wp_authenticate_username_password($user, $username, $password) {
		...
		if ( empty($username) || empty($password) || (!$is_mobile && isset($_POST['captcha_code']) && empty($_POST['captcha_code']))) {
		    $error = new WP_Error();
            ...
            if (!$is_mobile && isset($_POST['captcha_code']) && empty($_POST['captcha_code'])) {
