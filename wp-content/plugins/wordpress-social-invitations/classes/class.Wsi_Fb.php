    ...
    private $adapter;
 	...
 	function process(){
		...
		do_action('wsi_invitation_sent', $this->_user_id, $this->_wsi_obj_id);									
 		...
 		Wsi_Logger::log_stat('facebook',$this->_user_id, 1, $this->_id, $this->_display_name, $this->_wsi_obj_id );
 		
 		$wpdb->query("DELETE FROM {$wpdb->base_prefix}wsi_queue WHERE id ='$this->_id'");
 		
 		
 		$queue_data = $wpdb->get_row("SELECT id, sdata, friends, subject, message, send_at, i_count, user_id, display_name, wsi_obj_id FROM {$wpdb->base_prefix}wsi_queue WHERE provider = 'facebook' AND id > '$this->_id' ORDER BY id ASC LIMIT 1");
