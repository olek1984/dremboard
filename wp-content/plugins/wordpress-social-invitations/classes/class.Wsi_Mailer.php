...
class Wsi_Mailer{
	...
	private $limit;
 	...
 	public function __construct( $queue_data ,$total_sent = 0)
 	{
 		...
 		add_action( 'phpmailer_init', array($this, 'WCMphpmailerException'),20 );
 		
 		
 		if( $this->_options['send_with'] != 'own' )
 	...
 	function process(){
 		...
 		//save stats
 		Wsi_Logger::log_stat($this->_provider, $this->_user_id, $sent_on_batch, $this->_id, $this->_display_name, $this->_wsi_obj_id);
 		...
 		if( $this->_total_sent < $this->_limit )
 		{
 			...
 			$queue_data = $wpdb->get_row("SELECT id, sdata, friends, subject, message, send_at, i_count, user_id, display_name, provider, wsi_obj_id FROM {$wpdb->base_prefix}wsi_queue WHERE provider = 'google' OR provider = 'mail' OR provider = 'yahoo' OR provider = 'live' OR provider = 'foursquare' ORDER BY id ASC LIMIT 1");
	...
	function php_mailer_settings($phpmailer){
		...
		if( $this->_options['send_with'] == 'gmail' )
		{
		     $phpmailer->SMTPAuth   = true;  // Authentication
		     $phpmailer->Host       = 'smtp.gmail.com';
		     $phpmailer->Username   = $this->_options['gmail_username'];
		     $phpmailer->Password   = $this->_options['gmail_pass'];
		...
		{
		     $phpmailer->SMTPAuth   = true;  // Authentication
		     $phpmailer->Host       = $this->_options['smtp_server'];
		     $phpmailer->Username   = $this->_options['smtp_username'];
		     $phpmailer->Password   = $this->_options['smtp_pass'];
		     $phpmailer->SMTPSecure = $this->_options['smtp_secure']; // Enable if required - 'tls' is another possible value
		     $phpmailer->Port       = $this->_options['smtp_port'];    // SMTP Port		

		}
		if ( $this->_options['enable_dev'] == 'true' )
		{
