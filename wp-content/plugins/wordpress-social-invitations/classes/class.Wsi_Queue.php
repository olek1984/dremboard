...
if( !class_exists('Wsi_Queue') ) {
	...
	class Wsi_Queue{
		...
		function add_to_queue($provider, $sesion_data, $friends, $subject, $message, $display_name, $wsi_obj_id)
		{
			global $wpdb;
		...
		function process_queue()
		{
			...
			if( !$lock_fb )
			{
				...
				try{
					...
					$queue_data = $wpdb->get_row("SELECT id, sdata, friends, subject, message, user_id, send_at,i_count, display_name, wsi_obj_id FROM {$wpdb->base_prefix}wsi_queue WHERE provider = 'facebook' ORDER BY id ASC");
			...
			if ( !$lock_email )
			{
				...
				try{
					...
					$queue_data = $wpdb->get_row("SELECT id, sdata, friends, subject, message, send_at, i_count, user_id, display_name, provider, wsi_obj_id FROM {$wpdb->base_prefix}wsi_queue WHERE provider = 'google' OR provider = 'mail' OR provider = 'yahoo' OR provider = 'live' OR provider = 'foursquare' ORDER BY id ASC");
			...
			if( !$lock_tw )
			{
				...
				try{
					//we don't knwo about fb limits , so lets run a whole row
					$queue_data = $wpdb->get_row("SELECT id, sdata, friends, subject, message, send_at, user_id, i_count, display_name, wsi_obj_id FROM {$wpdb->base_prefix}wsi_queue WHERE provider = 'twitter' AND ( send_at is NULL OR send_at <= NOW() ) ORDER BY id ASC");
			...
			if( !$lock_lk )
			{
				...
				try{
					//we don't knwo about fb limits , so lets run a whole row
					$queue_data = $wpdb->get_row("SELECT id, sdata, friends, subject, message, send_at, user_id, i_count, display_name, wsi_obj_id FROM {$wpdb->base_prefix}wsi_queue WHERE provider = 'linkedin' AND ( send_at is NULL OR send_at <= NOW() ) ORDER BY id ASC");
