...
//!!!
			...
			{
				//var_dump("1033");
				add_action( 'bp_member_header_actions',    'bp_add_family_button',           5 );
			}
...
//!!1
			// Familys
			'accept_familyship' => 'bp_legacy_theme_ajax_accept_familyship',
			'addremove_family'  => 'bp_legacy_theme_ajax_addremove_family',
			'reject_familyship' => 'bp_legacy_theme_ajax_reject_familyship',
			...
			'get_single_activity_comments' => 'bp_legacy_theme_get_single_activity_comments',
			...
			// Notifications
			'notifications_get'           	=> 'bp_legacy_theme_ajax_notifications_get',
            'change_media_cover'            => 'bp_legacy_theme_ajax_change_media_cover',
            'see_more_trend'                => 'bp_legacy_theme_ajax_see_more_trend',
            //copy right.
            'submit_copy_right_compliant'   => 'bp_legacy_theme_ajax_submit_copy_right_compliant',
            'submit_copy_right_counter_claim'   => 'bp_legacy_theme_ajax_submit_copy_right_counter_claim',
            'submit_copy_right_dispute_counter'   => 'bp_legacy_theme_ajax_submit_copy_right_dispute_counter',
		);
...
//!!!
		case 'familys':
			$feed_url = bp_loggedin_user_domain() . bp_get_activity_slug() . '/familys/feed/';
			break;
		case 'groups':
		...
		case 'mentions':
            ...
            bp_notifications_mark_notifications_by_type( bp_loggedin_user_id(), 'activity', 'new_at_mention');
			break;
		case 'notifications':
			$feed_url = bp_loggedin_user_domain() . bp_get_activity_slug() . '/notifications/feed/';
            bp_activity_clear_new_mentions( bp_loggedin_user_id() );
            bp_notifications_mark_notifications_by_type( bp_loggedin_user_id(), 'activity', 'new_at_mention');
            break;
		default:
...
function bp_legacy_theme_post_update() {
	...
	if ($_POST['item_id'] == '-1' && empty( $_POST['content'] ) )
		exit( '-1<div id="message" class="error"><p>' . __( 'Please enter some content to post a trend.', 'buddypress' ) . '</p></div>' );

	$activity_id = 0;
	if ( empty( $_POST['object'] ) && bp_is_active( 'activity' ) ) {
        $trend_flag = false;
        if ($_POST['item_id'] == '-1'){ //Trending Drems
            $trend_flag = true;
            $_POST['item_id'] = '';
        }
		$activity_id = bp_activity_post_update( array( 'content' => $_POST['content'], 'trend_flag' => $trend_flag ) );
...
function bp_legacy_theme_new_activity_comment() {
	...
	$picture_flag = false;
	if (count($_FILES) > 0) $picture_flag = true;
	
	if ( ($picture_flag == false) && empty( $_POST['content'] ) )
	...
	if ($picture_flag){
		...
		$uploaded['privacy'] = "0";
		$uploaded['album_id'] = 1;
		$uploaded['title'] = $_POST['file_name'];
		$uploaded['description'] = $_POST['content'];
		$uploaded['media_author'] = bp_loggedin_user_id();
        ...
        $activity_content = $activity->create_activity_html ($_POST['content'], true);
...
function bp_legacy_theme_get_single_activity_comments() {
	// Bail if not a POST action
	if ( 'POST' !== strtoupper( $_SERVER['REQUEST_METHOD'] ) )
		return;
...
function bp_legacy_theme_ajax_addremove_family() {
	...
	// Cast fid as an integer
	$family_id = (int) $_POST['fid'];

	// Trying to cancel friendship
	if ( 'is_family' == BP_family_familyship::check_is_family( bp_loggedin_user_id(), $family_id ) ) {
		check_ajax_referer( 'familys_remove_family' );

		if ( ! familys_remove_family( bp_loggedin_user_id(), $family_id ) ) {
			echo __( 'familyship could not be canceled.', 'buddypress' );
		} else {
			echo '<a id="family-' . esc_attr( $family_id ) . '" class="add" rel="add" title="' . __( 'Add family', 'buddypress' ) . '" href="' . wp_nonce_url( bp_loggedin_user_domain() . bp_get_familys_slug() . '/add-family/' . $family_id, 'familys_add_family' ) . '">' . __( 'Add family', 'buddypress' ) . '</a>';
		}

	// Trying to request friendship
	} elseif ( 'not_familys' == BP_familys_familyship::check_is_family( bp_loggedin_user_id(), $family_id ) ) {
		check_ajax_referer( 'familys_add_family' );

		if ( ! familys_add_family( bp_loggedin_user_id(), $family_id ) ) {
			echo __(' familyship could not be requested.', 'buddypress' );
		} else {
			echo '<a id="family-' . esc_attr( $family_id ) . '" class="remove" rel="remove" title="' . __( 'Cancel familyship Request', 'buddypress' ) . '" href="' . wp_nonce_url( bp_loggedin_user_domain() . bp_get_familys_slug() . '/requests/cancel/' . $family_id . '/', 'familys_withdraw_familyship' ) . '" class="requested">' . __( 'Cancel familyship Request', 'buddypress' ) . '</a>';
		}

	// Trying to cancel pending request
	} elseif ( 'pending' == BP_familys_familyship::check_is_family( bp_loggedin_user_id(), $family_id ) ) {
		check_ajax_referer( 'familys_withdraw_familyship' );

		if ( familys_withdraw_familyship( bp_loggedin_user_id(), $family_id ) ) {
			echo '<a id="family-' . esc_attr( $family_id ) . '" class="add" rel="add" title="' . __( 'Add family', 'buddypress' ) . '" href="' . wp_nonce_url( bp_loggedin_user_domain() . bp_get_familys_slug() . '/add-family/' . $family_id, 'familys_add_family' ) . '">' . __( 'Add family', 'buddypress' ) . '</a>';
		} else {
			echo __("familyship request could not be cancelled.", 'buddypress');
		}
...
//!!!
function bp_legacy_theme_ajax_accept_familyship() {
	// Bail if not a POST action
	if ( 'POST' !== strtoupper( $_SERVER['REQUEST_METHOD'] ) )
		return;
	...
	if ( ! familys_accept_familyship( (int) $_POST['id'] ) )
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem accepting that request. Please try again.', 'buddypress' ) . '</p></div>';
...
//!!!
function bp_legacy_theme_ajax_reject_familyship() {
	// Bail if not a POST action
	if ( 'POST' !== strtoupper( $_SERVER['REQUEST_METHOD'] ) )
		return;
	...
	if ( ! familys_reject_familyship( (int) $_POST['id'] ) )
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem rejecting that request. Please try again.', 'buddypress' ) . '</p></div>';
...
function bp_legacy_theme_ajax_messages_autocomplete_results() {
	...
	function my_enqueue_styles() {
		...
		*/
		if(bp_is_familys_component())
		{
			...
			//wp_enqueue_style( $asset['handle'], $asset['location'], array(), $this->version, 'screen' );
			wp_enqueue_style('gavern-buddypress', 'buddypress.css', array('gavern-extensions'));
			wp_enqueue_style('gavern-buddypress', gavern_file_uri('css/my.gk.buddypress.css'), array('gavern-extensions'));
			return;
		}
	}
	//my_enqueue_styles();
	...
	function bp_legacy_theme_ajax_notifications_get(){
		$bp                    = buddypress();
		$notifications         = BP_Notifications_Notification::get( array(
								'user_id' => bp_loggedin_user_id()));
		$notifications_brief = array();
		$notification_obj = array();
		foreach($notifications as $notification){
			...
			$description = apply_filters( 'bp_get_the_notification_description', $description );
			
			/////////////////// time_since ///////////////////////////
			$date_notified = $notification->date_notified;
                        
                        $org_date = $date_notified;
                        if ( !empty( $org_date ) && !is_numeric( $org_date ) ) {
                            $time_chunks = explode( ':', str_replace( ' ', ':', $org_date ) );
                            $date_chunks = explode( '-', str_replace( ' ', '-', $org_date ) );
                            $org_date  = gmmktime( (int) $time_chunks[1], (int) $time_chunks[2], (int) $time_chunks[3], (int) $date_chunks[1], (int) $date_chunks[2], (int) $date_chunks[0] );
                        }
                        $cur_date = strtotime( bp_core_current_time() );
                        $interval = $cur_date - $org_date;
                        //if time interval is more 10 minutes
                        if ($interval > 10*60 || $interval < 0)
                        {
                            //continue;
                            $notification_obj['is_popupped'] = true;
                        }
                        else
                        {
                            $notification_obj['is_popupped'] = false;
                        }
			...
			$time_since = apply_filters( 'bp_get_the_notification_time_since', $retval );
                        
			////////////////// action_link /////////////////////////////////
			//$link_url = wp_nonce_url( add_query_arg( array( 'action' => 'read', 'notification_id' => $notification->id ), bp_get_notifications_unread_permalink() ), 'bp_notification_mark_read_' . $notification->id );
			/*<a href="<?php echo esc_url( $url ); ?>" class="mark-read primary"><?php _e( 'Read', 'buddypress' ); ?></a>*/	
			$notification_obj['id'] = $notification->id;
			$notification_obj['desc'] = $description;
			//$notification_obj['link'] = $link_url;
			$notification_obj['since'] = $time_since;
                        
			$notifications_brief[] = $notification_obj;
		}
		///////////////////////////////////////
		//$notifications = bp_notifications_get_notifications_for_user(bp_loggedin_user_id(), 'object');
		//$notifications = BP_Notifications_Notification::get( array('user_id' => bp_loggedin_user_id()	) );
		echo json_encode($notifications_brief);
	}
    
    function bp_legacy_theme_ajax_change_media_cover(){
        $ret_val = "false";
        if (isset($_POST['data']['media_id'])){
            $media_id = $_POST['data']['media_id'];

            $rtmedia_model = new RTMediaModel();
            $cover_art = $rtmedia_model->get_media_cover_art( $media_id );
            if ($cover_art > 0){
                $cover_art = 0;
                $ret_val = 'Not Cover';
            }else{
            ...
            $rtmedia_model->set_media_cover_art( $media_id,  $cover_art);
        }
        exit( json_encode( $ret_val ) );
    }
    
    function bp_legacy_theme_ajax_see_more_trend(){
        $ret_val = "false";
        if (isset($_POST['data']['page'])){
            $page = $_POST['data']['page'];
            $trend_html = get_trend_drems($page, $more_flag);
            $ret_val = array();
            $ret_val['more_flag'] = $more_flag;
            $ret_val['trend_html'] = $trend_html;
        }
        exit( json_encode( $ret_val ) );
    }
    
    function bp_legacy_theme_ajax_submit_copy_right_compliant(){
        $ret_val = array();
        $data = array();
        $case_ids = array();
        ...
        if (!bp_check_dcma_request_data($data, $msg)){
        ...
        $case_ids = bp_create_dcma_request($data, $agent_id);
        
        $ret_val['status'] = 'true';
        $ret_val['msg'] = "Once a claim has been submitted that photo/video will be taken down. That way we will stay in compliance with DMCA.";
        exit( json_encode( $ret_val ) );
    }

    function bp_legacy_theme_ajax_submit_copy_right_counter_claim(){
        $ret_val = array();
        $data = array();
        $case_ids = array();
        parse_str($_POST['data'], $data);
        ...
        // check values are valid.
        if (!bp_check_dcma_counter_claim_data($data, $msg)){
            $ret_val['status'] = 'false';
            $ret_val['msg'] = $msg;
            exit( json_encode( $ret_val ) );
        }
        
        bp_create_dcma_counter_claim($data);
        
        $ret_val['status'] = 'true';
        $ret_val['msg'] = "The party who orginally submitted Copyright Claim will be informed and that an email has been sent to your email address for your record";
        exit( json_encode( $ret_val ) );
    }
    
    function bp_legacy_theme_ajax_submit_copy_right_dispute_counter() {
        $ret_val = array();
        $data = array();
        $case_ids = array();
        parse_str($_POST['data'], $data);
        ...
        // check values are valid.
        if (!bp_check_dcma_dispute_counter_data($data, $msg)){
            $ret_val['status'] = 'false';
            $ret_val['msg'] = $msg;
            exit( json_encode( $ret_val ) );
        }
               
        bp_create_dcma_dispute_counter($data);
        
        $ret_val['status'] = 'true';
        $ret_val['msg'] = "The party who orginally submitted Copyright Counter Claim will be informed and that an email has been sent to your email address for your record";
        exit( json_encode( $ret_val ) );
    }