...
function gk_widget_control_init_events(id, inner) {
    ...
    if (form.attr('data-state') !== 'initialized') {
        ...
        select.change(function () {
            ...
            if (value === 'homepage' || value === 'buddypress' || value === 'page404' || value === 'search' || value === 'archive') {
    ...
    if (output.length > 0) {
        ...
        for (var i = 0; i < pages.length; i++) {
            if (pages[i] !== '') {
                pages_exist = true;
                var type = 'homepage';
                var types = ['page:', 'post:', 'category:', 'category_descendant:', 'tag:', 'archive', 'author:', 'template:', 'taxonomy:', 'posttype:', 'page404', 'search', 'buddypress'];
